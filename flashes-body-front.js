document.addEventListener('DOMContentLoaded', () => {
  const grid = document.getElementById('flashesBodyFrontGrid');
  const backLink = document.getElementById('flashesBodyFrontBack');
  ...
  if (!grid || !window.BODY_FRONT_ZONES) return;

  const imgV = window.FLASHES_IMG_V || '82';

  function previewUrl(path) {
    return path.split('/').map((part) => encodeURIComponent(part)).join('/') + `?v=${imgV}`;
  }

  window.BODY_FRONT_ZONES.forEach((zone) => {
