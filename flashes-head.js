document.addEventListener('DOMContentLoaded', () => {
  const grid = document.getElementById('flashesHeadGrid');
  const backLink = document.getElementById('flashesHeadBack');
  ...
  if (!grid || !window.HEAD_ZONES) return;

  const imgV = window.FLASHES_IMG_V || '89';

  function previewUrl(path) {
    return path.split('/').map((part) => encodeURIComponent(part)).join('/') + `?v=${imgV}`;
  }

  window.HEAD_ZONES.forEach((zone) => {
